...
**Interactive docs as an attack surface.** FastAPI's default `/docs` (Swagger UI) includes a "Try it out" button that sends real, live requests against production data — confirmed directly exploitable on `DELETE /api/plans/{plan_id}` and any other unauthenticated mutating endpoint, since it requires no special tooling, just a browser. This doesn't introduce new risk beyond the already-disclosed lack of per-endpoint authentication (anyone could already do the same with `curl`), but it materially lowers the skill floor to exploit it. `/docs` is now disabled (`docs_url=None`); `/redoc` remains available as a read-only API reference with no execution capability.
